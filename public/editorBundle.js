...
exports.Editor = {
    ...
    init: function () {
        ...
        input.view.x = map.w / 2;
        input.view.y = map.h / 2;
    },
    /**
    ...
    update: function () {
        if (input.mouse.browser.x > 276) {
            var px = input.mouse.mapX;
            var py = input.mouse.mapY;
    ...
    calculateShadows: function () {
        for (var x = 0; x < map.w; x++) {
            for (var y = 0; y < map.h; y++) {
                ...
                if (tile.tex && tile.tex.startsWith('floor')) {
                    tile.shadow = null;
                    var pCL = { x: clamp(x - 1, 0, map.w - 1), y: y };
                    var pBL = { x: clamp(x - 1, 0, map.w - 1), y: clamp(y + 1, 0, map.h - 1) };
                    var pBC = { x: x, y: clamp(y + 1, 0, map.h - 1) };
    ...
    paint: function (px, py) {
        ...
        var pTL = { x: clamp(px - 1, 0, map.w - 1), y: clamp(py - 1, 0, map.h - 1) };
        var pTC = { x: px, y: clamp(py - 1, 0, map.h - 1) };
        var pTC2 = { x: px, y: clamp(py - 2, 0, map.h - 1) };
        var pTR = { x: clamp(px + 1, 0, map.w - 1), y: clamp(py - 1, 0, map.h - 1) };
        var pCL = { x: clamp(px - 1, 0, map.w - 1), y: py };
        var pCL2 = { x: clamp(px - 2, 0, map.w - 1), y: py };
        var pCR = { x: clamp(px + 1, 0, map.w - 1), y: py };
        var pCR2 = { x: clamp(px + 2, 0, map.w - 1), y: py };
        var pBL = { x: clamp(px - 1, 0, map.w - 1), y: clamp(py + 1, 0, map.h - 1) };
        var pBC = { x: px, y: clamp(py + 1, 0, map.h - 1) };
        var pBC2 = { x: px, y: clamp(py + 2, 0, map.h - 1) };
        var pBR = { x: clamp(px + 1, 0, map.w - 1), y: clamp(py + 1, 0, map.h - 1) };
    ...
    menuSetup: function () {
        ...
        */
        ...
        exports.Editor.addEditorItem([
            ...
            { block_id: "block_file", type: "button", value: "Save", onclick: "Editor.saveData(map.data, 'map.json')" }
        ]);
